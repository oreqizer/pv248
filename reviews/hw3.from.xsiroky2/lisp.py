...
## The code is overall good, there are very little details to mention
## I like that you are using exceptions and determining None return from that
## It makes life much easier for debugging
def parse(s):
    try:
        tokens = tokenize(s)
...
## Probably leftover from your custom testing
## should be omitted in submited solution
def parse_test(s):
...
def tokenize(s):
    ...
    ## Depth should be name for numeric variable, "nesting" could be more fitting name
    depth = []         # ( ) or [ ] nesting
    ...
    while len(expr) > 0:
	    ...
	    ## Maybe just calling recursively parse on internal expr would be simplier
	    ## however your solution potentially runs faster.
            # Paren leave
            if len(depth) == 0:
