...
class State:
    ...
    def join(self, user, channel_name):
        ## Instead of this, you should definitely use something like:
        ##
        ## ch = self.channels.get(channel_name, None)
        ## if ch is None:
        ## ...
        ##
        ## It would be shorter and way more efficient.
    ...
    def get_channel(self, name):
        ## An almost a same thing as above (in join).
...
class Channel:
    def __init__(self, name):
        self.name = name
        self.messages = []
        ## nice, I have not used set for a long time.
        self.users = set()
...
## According to hw3 specs, `Compound` should not be a special case of atom.
## I personally defined an Expr class as a superclass to Atom and Compound.
class Compound(Atom):
    ...
    ## Defining only __iter__ method with "return self.value.__iter__()" would
    ## be sufficient (no __next__ needed) (list already has __iter__ defined).
    def __iter__(self):
        self.number = 0
        return self
...
class Number(Literal):
    ...
    ## I would import operator module and replace these lambda functions
    ## with operator.eq and others.
    def __eq__(self, o):
...
class String(Literal):
    ...
    ## I learned just today from reviewing other student's hw4 that one can put
    ## just "__str__ = __repr__" instead of this ;).
    def __str__(self):
        return f'"{repr(self)}"'
