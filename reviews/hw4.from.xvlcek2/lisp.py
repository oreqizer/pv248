...
async def parse(reader):
    ...
    ## I like that you often define a special variable to make it more readable.
    iden = root[0]
...
## I would personally rather create classes like Ok/Error with some method like
## "prepare_to_send()" returning a string with '\n', but this works too.
def make_ok():
    # (ok)
...
def is_channel(s):
    ...
    if hashtag != '#':
        return False
    ## You should replace this whole chunk with "return s[1:].isalnum()"
    for c in s[1:]:
        if not c.isalnum():
            return False
    return True
...
## As far as I know, this should not be called `token`. The token
## is an object that already carries information like a type (e.g.
## NUMBER, STRING, ...). This could be called `attribute` (which is used
## in automata theory).
##
## Then if this `attribute` is useful (e.g. token type is NUMBER), you call
## it a `lexeme` and make it part of a token (example: a token
## with a type "IDENT" and lexeme "my_var".
##
## Most tokens have their `lexeme` associated as an attribute, some don't
## need it (e.g. brackets, commas). But yes, parsing theory is a bit vague.
def parser(token):
    if token == '':
        return Atom('')
...
## Can't find any reason for using a `set` here. If you'd put something there
## twice, nothing would happen anyway. Using tuple only would be way faster.
id_symbol = set(['!', '$', '%', '&', '*', '/',
                 ':', '<', '=', '>', '?', '_', '~'])
id_special = set(['+', '-', '.', '@', '#'])
