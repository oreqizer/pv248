...
## Overall seems you're pretty proficient in Python and don't like NumPy. I
## like the object-oriented approach here. The code is overall pretty clean so
## the review might be a bit lackluster.
import numpy as np
...
def eval_root(root):
    ...
    ## seems a bit redundant since you check the arg count in the eval_*
    ## functions as well
    if len(root) < 2:
        raise Exception(
...
## The class-oriented approach is interesting but using numpy methods more
## would probably make your life much easier(mainly in Matrix)
# === CLASSES ===
...
class Matrix:
    def __init__(self, values):
        # values: [Vector]
        ## Wouldn't it just be simpler to use a numpy array instead of a list
        ## of Vectors to avoid all the
        ## [Vector(list(r)) for r in *numpy method returning ndarray*]?
        self.values = values
        self.x = len(values[0])
        self.y = len(values)
    ...
    def solve(self):
        ...
        if res.size == 0:
            ## using np.zeros() would be simpler here
            res = np.array([0 for x in range(self.y)])
        else:
            res_vect = []
            ## you can get a column by slicing numpy arrays(the indexing
            ## admiteddly gets a bit confusing in more dimensions) which would
            ## be cleaner here i think
