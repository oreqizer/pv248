## the load() function should be separated into more functions,
## but overall very nice - my code was very messy and all
## over the place, hard to read...
## I really appreciate the code style and
## query handling - something I failed in
...
def store(shelter, *, db, deduplicate=False):
    ...
    # Dedupe
    ## wow, mine was complex like sh*t
    ## (though more effective :P)
    if deduplicate:
...
## love the separation idea, mine was just stuffed in code
...
## just don't. use functions, not spaghetti
def load(id, *, db):
...
def insert_shelter(db, shelter):
    ...
    db.commit()
    ## cursor has last_rowid (or something simmilar) attribute that
    ## has the last insertion id value...
...
# === INIT ===
## id attribute is present by default, no need to create one
## (actually, this is an rowid->id alias creation that already exists)
ddl_init = """
--begin-sql
